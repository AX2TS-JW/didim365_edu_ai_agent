...
부동산 실거래가 분석 AI 에이전트 — 자연어로 부동산 질문을 하면 data.go.kr API에서 실거래가 데이터를 조회하고, PDF 보고서에서 전문가 분석을 검색하여 종합 답변합니다. LangChain ReAct 에이전트(메인) + LangGraph StateGraph 서브에이전트(PDF 하이브리드 검색) 구조. 모든 코드는 `edu_ai_agent/` 하위에 위치합니다.
...
Backend (`agent/.env`): `OPENAI_API_KEY`, `OPENAI_MODEL` (default: gpt-4.1), `DATA_GO_KR_API_KEY` (data.go.kr 디코딩키), `CORS_ORIGINS`, `API_V1_PREFIX`, `AGENT_MODE` (default: react, 옵션: react/graph)
...
- **Services**: `agent/app/services/agent_service.py` — ReAct 에이전트 스트리밍을 SSE 형식(model→tools→done)으로 변환 (기본). `agent/app/services/graph_agent_service.py` — StateGraph 전체 에이전트용 (`AGENT_MODE=graph`일 때)
- **Agent (ReAct, 기본)**: `agent/app/agents/real_estate_agent.py` — `create_agent()` + `ToolStrategy(ChatResponse)` + StateGraph 서브에이전트(`search_pdf_reports` 도구: BM25+Vector 병렬 검색). `AGENT_MODE=react`(기본)
- **Agent (StateGraph, 대안)**: `agent/app/agents/real_estate_graph.py` — 전체 흐름을 StateGraph로 제어. 노드: intent→parse→fetch→search_pdf→respond. `AGENT_MODE=graph`로 전환 가능
...
- **ReAct 메인 + StateGraph 서브에이전트**: `real_estate_agent.py`의 ReAct 에이전트가 기본. PDF 검색(`search_pdf_reports`)은 StateGraph 서브에이전트(BM25+Vector 병렬→merge→format)를 `@tool`로 래핑하여 ReAct 도구로 등록
- **AGENT_MODE 환경변수**: `react` (기본, ReAct + StateGraph 서브에이전트) 또는 `graph` (전체 StateGraph). `chat.py`에서 분기
- **graph 모드 영속 체크포인트**: `AsyncSqliteSaver`로 대화 히스토리 영속 저장 (서버 재시작 후에도 맥락 유지). react 모드는 `InMemorySaver`
- **graph 모드 trim_messages**: `MAX_MESSAGE_TOKENS=4000`으로 토큰 폭증 방지. 오래된 메시지부터 잘라냄
- **graph 모드 comprehensive 경로**: intent→parse→fetch→search_pdf→respond. PDF 보고서 + 실거래가를 종합 분석. 실거래가는 요약본만 전달하여 토큰 절약
...
- `search_pdf_reports` 도구: `real_estate_agent.py`에 정의. StateGraph 서브에이전트(BM25+Vector 병렬→merge→format)를 `@tool`로 래핑. `pipeline/search.py`의 `search_bm25`, `search_vector` 사용
